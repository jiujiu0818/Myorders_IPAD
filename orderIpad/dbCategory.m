...
+ (NSString *)findByID:(NSString *)ID
{
	...
	id<PLResultSet> rs;
	rs = [dataBase executeQuery:[NSString stringWithFormat: @"SELECT Name FROM Category where ID = %@",ID]];
	
	//定义一个数组存放所有categpry的信息
    NSString *categoryName = [[NSString alloc] init];
	
	//把rs中的数据库信息遍历到books数组
	while ([rs next]) {
        categoryName = [rs objectForColumn:@"Name"];
	}
	//关闭数据库
	[rs close];
    return categoryName;
