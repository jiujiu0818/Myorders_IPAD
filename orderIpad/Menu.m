...
+ (int) createWithID:(NSString *) ID name:(NSString *) name price:(NSString *)price categoryID:(NSString *)categoryID intro:(NSString *)intro
{
	PLSqliteDatabase *dataBase = [DataBase setup];
    
    BOOL bResult = [dataBase executeUpdate: @"INSERT INTO Menu (ID, Name, Price,categoryID,Intro,scoreCount,allScore,Pic) VALUES (?,?,?,?,?,1,3,'defaultDish')",
                    ID,
                    name,
                    price,
                    categoryID,
...
+ (int) update:(NSString *)ID withName:(NSString *)name price:(NSString *)price intro:(NSString *)intro{
	PLSqliteDatabase *dataBase = [DataBase setup];
    
    BOOL bResult = [dataBase executeUpdate: @"UPDATE Menu SET name = ?, price = ?, intro = ? where ID = ?",
                    name,
					price,
					intro,
					ID];
